...
$ResourceFiles | ForEach-Object {
    if ((Get-FileHash -Path $_.FullName).Hash -ne $FileHashes.$($_.Name)) {
        ...
        New-WPFMessageBox -Content $Content -Title 'Security Error' -TitleBackground Red -TitleTextForeground Black -TitleFontSize 20 -TitleFontWeight Bold -BorderThickness 1 -BorderBrush Black -Sound 'Windows Exclamation'
        Break
    }
}
